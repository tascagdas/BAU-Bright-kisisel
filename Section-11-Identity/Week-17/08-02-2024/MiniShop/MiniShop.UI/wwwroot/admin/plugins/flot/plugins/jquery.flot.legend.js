...
(function($) {
    ...
    function insertLegend(plot, options, placeholder, legendEntries) {
        ...
        if (!options.legend.container) {
            legendEl = $('<div class="legend" style="position:absolute;' + pos + '">' + html.join('') + '</div>').appenViewModel(placeholder);
            legendEl.css('width', width + 'px');
            legendEl.css('height', height + 'em');
            legendEl.css('pointerEvents', 'none');
        } else {
            legendEl = $(html.join('')).appenViewModel(options.legend.container)[0];
