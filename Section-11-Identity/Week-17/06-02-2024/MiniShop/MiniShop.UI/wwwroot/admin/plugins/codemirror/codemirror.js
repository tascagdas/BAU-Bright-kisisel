...
(function (global, factory) {
  ...
  function processLine(cm, text, context, startAt) {
    ...
    while (!stream.eol()) {
      reaViewModelken(mode, stream, context.state);
  ...
  function reaViewModelken(mode, stream, state, inner) {
    for (var i = 0; i < 10; i++) {
  ...
  function takeToken(cm, pos, precise, asArray) {
    ...
    while ((asArray || stream.pos < pos.ch) && !stream.eol()) {
      stream.start = stream.pos;
      style = reaViewModelken(mode, stream, context.state);
  ...
  function runMode(cm, text, mode, context, f, lineClasses, forceToEnd) {
    ...
    while (!stream.eol()) {
      if (stream.pos > cm.options.maxHighlightLength) {
        ...
        style = extractLineClasses(reaViewModelken(mode, stream, context.state, inner), lineClasses);
      }
      if (inner) {
  ...
  function removeReadOnlyRanges(doc, from, to) {
    ...
    for (var i = 0; i < markers.length; ++i) {
      ...
      for (var j = 0; j < parts.length; ++j) {
        ...
        var newParts = [j, 1], dfrom = cmp(p.from, m.from), ViewModel = cmp(p.to, m.to);
        if (dfrom < 0 || !mk.inclusiveLeft && !dfrom) { newParts.push({ from: p.from, to: m.from }); }
        if (ViewModel > 0 || !mk.inclusiveRight && !ViewModel) { newParts.push({ from: m.to, to: p.to }); }
  ...
  function buildLineContent(cm, lineView) {
    ...
    for (var i = 0; i <= (lineView.rest ? lineView.rest.length : 0); i++) {
      ...
      builder.pos = 0;
      builder.adViewModelken = builViewModelken;
      // Optionally wire in some hacks into the token-rendering
      // algorithm, to deal with browser quirks.
      if (hasBadBidiRects(cm.display.measure) && (order = getOrder(line, cm.doc.direction))) { builder.adViewModelken = builViewModelkenBadBidi(builder.adViewModelken, order); }
      builder.map = [];
  ...
  function builViewModelken(builder, text, style, startStyle, endStyle, css, attributes) {
    if (!text) { return }
  ...
  // right-to-left text.
  function builViewModelkenBadBidi(inner, order) {
  ...
  function insertLineContent(line, builder, styles) {
    ...
    if (!spans) {
      for (var i$1 = 1; i$1 < styles.length; i$1 += 2) { builder.adViewModelken(builder, allText.slice(at, at = styles[i$1]), interpretTokenStyle(styles[i$1 + 1], builder.cm.options)); }
      return
    }
      ...
      while (true) {
        if (text) {
          ...
          if (!collapsed) {
            ...
            builder.adViewModelken(builder, tokenText, style ? style + spanStyle : spanStyle,
  ...
  function adViewModelScrollTop(cm, top) {
    if (top == null) { return }
  ...
  function adjustScrollWhenAboveVisible(cm, line, diff) {
    if (heightAtLine(line) < ((cm.curOp && cm.curOp.scrollTop) || cm.doc.scrollTop)) { adViewModelScrollTop(cm, diff); }
  }
  ...
  function addLineWidget(doc, handle, node, options) {
    ...
    changeLine(doc, handle, "widget", function (line) {
      ...
      if (cm && !lineIsHidden(doc, line)) {
        ...
        if (aboveVisible) { adViewModelScrollTop(cm, widget.height); }
        cm.curOp.forceUpdate = true;
      }
      return true
    });
  ...
  function markText(doc, from, to, options, type) {
    ...
    if (marker.adViewModelHistory) { addChangeToHistory(doc, { from: from, to: to, origin: "markText" }, doc.sel, NaN); }
    ...
    CodeMirror.prototype = {
      ...
      moveV: methodOp(function (dir, unit) {
        ...
        doc.extendSelectionsBy(function (range) {
          ...
          if (unit == "page" && range == doc.sel.primary()) { adViewModelScrollTop(this$1, charCoords(this$1, pos, "div").top - headPos.top); }
          return pos
        }, sel_move);
        if (goals.length) {
  ...
  function findPosH(doc, pos, dir, unit, visually) {
    ...
    function moveOnce(bounViewModelLine) {
      var next;
      if (unit == "codepoint") {
      ...
      if (next == null) {
        if (!bounViewModelLine && findNextLine()) { pos = endOfLine(visually, doc.cm, lineObj, pos.line, lineDir); }
        else { return false }
      } else {
        pos = next;
      }
  ...
  ContentEditableInput.prototype.pollContent = function () {
    ...
    var newTop = newText[0], olViewModelp = oldText[0], maxCutFront = Math.min(newTop.length, olViewModelp.length);
    while (cutFront < maxCutFront && newTop.charCodeAt(cutFront) == olViewModelp.charCodeAt(cutFront)) { ++cutFront; }
